...
// Réutiliser le même composant de signature que pour le CheckIn
const SignaturePad = ({ onSave, onClear, canvasRef }) => {
  ...
  return (
    <div className="space-y-3">
      <canvas
        ref={canvasRef} className="w-full h-48 border-2 border-slate-300 rounded-lg bg-white/80 cursor-crosshair touch-none"
...
export default function CheckOutStep3({ reservation, vehicle, client, organization, checkOutData, updateCheckOutData, onBack, onComplete }) {
  ...
  const handleSaveOwnerSignature = (signatureData) => {
    updateCheckOutData({ owner_signature: signatureData });
    setOwnerSigned(true);
  };

  const handleSaveClientSignature = (signatureData) => {
    updateCheckOutData({ client_signature: signatureData });
    setClientSigned(true);
  };

  const handleClearOwnerSignature = () => {
    setOwnerSigned(false);
    updateCheckOutData({ owner_signature: null });
  };

  const handleClearClientSignature = () => {
    setClientSigned(false);
    updateCheckOutData({ client_signature: null });
  };
  ...
  return (
    <div className="space-y-6">
      {/* Signature du loueur */}
      <Card className="bg-transparent text-card-foreground rounded-xl border backdrop-blur-lg border-white/40 shadow-lg">
        <CardHeader>
          ...
          <p className="text-sm text-slate-800 dark:text-slate-200">{organization?.name || "Loueur"}</p>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
              <div className="border-2 border-green-300 rounded-lg bg-green-50 p-4">
                <img src={checkOutData.owner_signature} alt="Signature loueur" className="w-full h-32 object-contain" />
              </div>
              <Button
              type="button"
      ...
      <Card className="bg-transparent text-card-foreground rounded-xl border backdrop-blur-lg border-white/40 shadow-lg">
        <CardHeader>
          ...
          <p className="text-sm text-slate-800 dark:text-slate-200">{client?.name || "Client"}</p>
        </CardHeader>
        <CardContent className="space-y-4">
          {!clientSigned ?
          <SignaturePad
          ...
          <div className="space-y-3">
              <div className="border-2 border-green-300 rounded-lg bg-green-50 p-4">
                <img src={checkOutData.client_signature} alt="Signature client" className="w-full h-32 object-contain" />
              </div>
              <Button
              type="button"
